...
const Conformers = {
    ...
    literal(tile) { return tile },
    /** Looks up the tile in the given array -- using [], so it will also technically work on objects. */
    Array(array) { return (tile) => array[tile] },
    /** Inhibits repeated calls to the same transition */
    once(conformer) { return (entry, oldTile) => entry.tile == oldTile ? entry.go : conformer(entry, oldTile) },
    ...
    Frame(lookup) { return ({go, tile}) => go.setFrame(lookup(tile)) },
    /** Sets the lookup(tile) as the first parameter to setTexture. */
    Texture(lookup) { return ({go, tile}) => go.setTexture(lookup(tile)) },
    /** Sets the lookup(tile) as ...parameters to setTexture. */
    TextureFrame(lookup) { return ({go, tile}) => go.setTexture(...lookup(tile)) },
    /** Sets the lookup(tile), ...args as an animation via play(). */
    Animation(lookup, ...args) { return ({go, tile}) => go.play(lookup(tile), ...args) },
};
