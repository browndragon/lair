...
import {ShadowPool} from '@browndragon/pool';

/** A pool that exports tiles which are "pressure sensitive", with outer corners red and inner corners green. */
export default class TestPool extends ShadowPool {
    constructor(scene) {
        super(scene);
        this.setConformer(new.target.conform);
    ...
    static conform({go, tile, u, v}, oldTile) {
        if (tile == oldTile) {
            // We're fine.
            return;
        }
