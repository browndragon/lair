...
export default class Spring {
    constructor() {
        ...
        this.scratch = Phaser.Math.Vector2.ZERO.clone();
        // Records the last-known values.
        this.impulse = 0;
        this.length = 0;
        // A bit of a misnomer -- this ends up being the actual directional vector of impulse, and is kind of crazy. Do not look directly at internal state transfer mechanism.
        this.displacement = Phaser.Math.Vector2.ZERO.clone();
        // "absolute displacement in X and Y"
        this.lX = 0;
        this.lY = 0;
    }
    ...
    set b(v) { return this.setB(v) }

    /** @return A's position plus its offset. */
    get aPos() {
        return this.scratch.setTo(
            this.a.body.x + this.aOffset.x * this.a.body.width,
            this.a.body.y + this.aOffset.y * this.a.body.height,
        );
    }
    /** @return B's position plus its offset. */
    get bPos() {
        return this.scratch.setTo(
            this.b.body.x + this.bOffset.x * this.b.body.width,
            this.b.body.y + this.bOffset.y * this.b.body.height,
        );
    }
    ...
    recalculateConstants() {
        ...
        this.reducedMass = (productMass || 1) / (sumMass || 1);
        // console.log('Reduced mass:', this.aMass, '&', this.bMass, '=', this.reducedMass);
    }
    ...
    prepare(delta) {
        ...
        this.impulse = this.reducedMass * (
            1/seconds * this.k * (this.length - this.l)
            + this.d * relativeVelocity
        );
        // Maybe I should rename this, since it's kind of a bad name.
        this.displacement.setLength(this.impulse);
        // And this in particular does us no favors...: this.impulse /= this.reducedMass;
        // console.groupEnd();
        return true;
    }
    execute() {
