export function setDataFromTile(entity, tile, layer, tilemap) {
...
export function setDataFromObject(entity, object, layer, tilemap) {
    // Set data & properties from object
    setDataFromLayerAndMap(entity, layer, tilemap);
    setDataFromTileset(entity, object.gid, tilemap)
    ...
    entity.angle = object.rotation;
    // Then: see ParseObject:
    entity.flipX = object.flippedHorizontal;
    entity.flipY = object.flippedVertical;
    // template?
    // text?
...
function setDataFromLayerAndMap(entity, layer, tilemap) {
    ...
    entity.alpha = layer.opacity;
    // Paralaxx & y?
    if (layer.tintcolor) { entity.setTint(Phaser.Display.Color.HexStringToColor(layer.tintcolor).color) }
