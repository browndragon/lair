 ...
 */
export default function facer(clazz, asSuffix=true) {
    if (clazz[F]) { return clazz }
    return class extends clazz {
        static get [F]() { return true }
        constructor(...params) {
            super(...params);
...
function quadrant(rotation) {
    ...
    const reoriented = Phaser.Math.Wrap(
        rotation + Phaser.Math.TAU / 2, 0, Phaser.Math.PI2
    );
    // Then divide on the quadrants.
    ...
    switch (quadrant) {
        ...
        case 2: return 'ww';
        case 3: return 'nn';
        default: throw new TypeError(`Unrecognized quadrant ${quadrant} for ${rotation}`);
    }
}