...
class Cursor {
  static get(scene) {
    return scene[C] || this.installInto(scene);
  }

  static installInto(scene) {
    return scene[C] = new this(scene).install();
  }
  ...
  install() {
    ...
    this.dirs = new Map().set(keys.left, Phaser.Math.Vector2.LEFT).set(keys.right, Phaser.Math.Vector2.RIGHT).set(keys.up, Phaser.Math.Vector2.UP).set(keys.down, Phaser.Math.Vector2.DOWN).set(keys.a, Phaser.Math.Vector2.LEFT).set(keys.d, Phaser.Math.Vector2.RIGHT).set(keys.w, Phaser.Math.Vector2.UP).set(keys.s, Phaser.Math.Vector2.DOWN);
