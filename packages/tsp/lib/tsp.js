...
var _importLambdas = _interopRequireDefault(require("./importLambdas"));
...
class TSP extends Phaser.Plugins.ScenePlugin {
  boot() {
    ...
    events.on('start', this.sceneStart, this); // events.on('update', this.sceneUpdate, this);
  ...
  sceneStart() {
    ...
    this.systems.updateList.on('remove', this.remove, this); // if (this.scene.update) { this.updaters.add(this.scene) }    // Handled by Phaser.

    if (this.scene.postUpdate) {
      this.objs.postUpdate.set(this.scene);
    }

    if (this.scene.tick) {
      this.objs.tick.set(this.scene);
    }

    if (this.scene.postTick) {
      this.objs.postTick.set(this.scene);
    }
  ...
  add(obj) {
    obj.once('destroy', this.remove, this); // if (obj.update) { this.objs.updaters.set(obj) }

    if (obj.postUpdate) {
      this.objs.postUpdate.set(obj);
    }

    if (obj.tick) {
      this.objs.tick.set(obj);
    }

    if (obj.postTick) {
      this.objs.postTick.set(obj);
    }
...
TSP.preload = new _importLambdas.default();
/** Adds a lambda to be run when all creates are run. */

TSP.create = new _importLambdas.default();