...
export default function destructureRight(array, rightElements=1, leftElements=0) {
    const rightElementsAvailable = Math.min(
        rightElements,
        array.length);
    const leftElementsAvailable = Math.max(
        0,
        Math.min(leftElements, array.length - rightElementsAvailable)
    );
    return [
        ...Array.from(
            {length:leftElements},
            (_, i) => i < leftElementsAvailable ? array[i] : undefined
        ),
        array.slice(leftElementsAvailable, Math.max(0, array.length - rightElementsAvailable)),
        ...array.slice(array.length - rightElementsAvailable)
    ];
}
