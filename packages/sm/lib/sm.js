...
class SM {
  ...
  constructor(statesObject) {
    ...
    console.assert(statesObject); // You're not allowed to handle the undefined key.

    console.assert(!(undefined in statesObject)); // The object being driven. Key -> function(stateMachine, ...params).
    ...
    this.states = statesObject; // The last completely run state method. DURING a state call, this will be the previously executed run state method.

    this.prev = undefined; // The next state method to run. Has not completed execution yet.
  ...
  step(...params) {
    ...
    while (this._increment() && this.prev != this.next) {// The incrementation & execution occurs during _increment.
  ...
  _increment() {
    ...
    const needsAnotherCall = state.call(this.states, this, ...this.params); // if the previous shifted out from under us, there was a concurrent modification. Horrifying. Abort.

    console.assert(this.prev == prevWas); // We expect/allow next to move, though, so use the copy we grabbed.
