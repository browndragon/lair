 ...
 */
class PGroup extends (0, _registrar.default)(Phaser.Physics.Arcade.Group) {
  constructor(scene) {
  ...
  static install(scene) {
    ...
    if (colliders) {
      ...
      scene.physics.add.collider(group, colliders.map(g => assertSafe(g.group(scene))), this.collider, this.process, this);
    }
    ...
    if (overlaps) {
      ...
      scene.physics.add.overlap(group, overlaps.map(g => assertSafe(g.group(scene))), this.overlap, this.process, this);
    }
  ...
  static overlap(a, b) {} // Used for both collider() and overlap(), process() can cancel a collision (such as if you should break through an enemy instead of bouncing).
...
exports.default = PGroup;
;

function assertSafe(g) {
  console.assert( // Relies on someone else already importing Phaser for us. We can't do it here, because npm imports mean we'd get a different definition of Phaser than the invoker would!
  g instanceof Phaser.Physics.Arcade.Group || g instanceof Phaser.Physics.Arcade.StaticGroup);
  return g;
}